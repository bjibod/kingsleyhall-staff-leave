...
export async function submitLeaveRequest(_: LeaveActionState, formData: FormData): Promise<LeaveActionState> {
  const parsed = leaveRequestSchema.safeParse(Object.fromEntries(formData)); if (!parsed.success) return { error: "Check the dates and leave type." };
  try {
    const { user, pattern, year, available } = await requestContext();
    const leaveType = await db.leaveType.findFirst({ where: { id: parsed.data.leaveTypeId, organisationId: user.employee!.organisationId, active: true } });
...
export async function reviewLeaveRequest(formData: FormData) {
  ...
  const parsed = leaveDecisionSchema.safeParse(Object.fromEntries(formData)); if (!parsed.success) throw new Error("Invalid decision");
